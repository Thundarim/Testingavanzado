...
router.get("/carts/:cid", async (req, res) => {
    try {
        ...
        req.logger.error("Error al obtener carrito por ID:", error);
        res.status(500).json({ error: "Error interno del servidor" });
    }
});
...
router.put("/carts/:cid", async (req, res) => {
    ...
    try {
        ...
        req.logger.error("Error al actualizar productos del carrito", error);
        res.status(500).json({ error: "Error interno del servidor" });
    }
});
...
router.post("/carts/:cid/purchase", async (req, res) => {
    ...
    try {
        ...
        if (!productsInCart || productsInCart.length === 0) {
            req.logger.log(`El carrito con ID ${req.params.cid} está vacío. Agrega productos antes de proceder con la compra.`);
        ...
        transporter.sendMail(mailOptions, (error, info) => {
            if (error) {
                ...
                req.logger.log('Correo enviado:', info.response);
            }
        });
...
router.post('/carts/add', async (req, res) => {
    try {
      ...
      req.logger.error('Error al añadir producto:', error);
      res.status(500).json({ error: 'error interno del servidor' });
    }
  });
