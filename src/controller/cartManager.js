...
async updateProductQuantity(cartId, productId, quantity) {
  try {
    ...
    if (!product) {
      throw new Error(`Producto con la id ${productId} no se encuentra en el carrito`);
    }

    product.quantity = quantity;
    await cart.save();

    console.log(`Cantidad del producto ${productId} actualizada a ${quantity} en el carrito ${cartId}.`);

    return cart;
  } catch (error) {
    console.log(`Error al actualizar la cantidad del producto ${productId} en el carrito ${cartId}:`, error);
    throw error;
  }
}
